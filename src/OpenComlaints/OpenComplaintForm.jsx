...
import ComplaintFormTabs from "@/components/LodgeComplaint/ComplaintFormTabs";
import AllegedPeronsForm from "@/components/LodgeComplaint/Steps/AllegedPersonsForm";
import ComplaintDetailsForm from "@/components/LodgeComplaint/Steps/ComplaintDetailsForm";
...
import { openComplaintSchema } from "@/schema/complaintSchema";
import { setActiveTabItem } from "@/stores/slices/authSlice";
...
const LodgeOpenComplaintMainForm = () => {
    ...
    const methods = useForm({
        resolver: yupResolver(openComplaintSchema),
        mode: "all",
        defaultValues: {
            fullName: "",
            cnic: "",
    ...
    const goToNext = async () => {
        ...
        if (activeTab === "step1") {
            currentStepFields = ["fullName", "fatherName", "gender", "cnic", "email", "phoneNumber", "address", "cityId","country"];
    ...
    const handleSubmitButton = async (data = {}) => {
        ...
        const witnesses = data?.witnessName && data?.witnessCnic ? [
            {
                name: data?.witnessName,
                cnic: data?.witnessCnic,
            },
        ] : [];

        formData.append("witnesses", JSON.stringify(witnesses));

        // Prepare and stringify allegedPersons array for backend
        const transformedAllegedPersons = data?.allegedPersons.map((p) => ({
            fullName: p.name || "",
    ...
    const handleLodgeComplaint = async (otp) => {
        if (!pendingPayload) return;
        setIsSubmitting(true);
        pendingPayload?.set("otp", otp);
        const response = await apiHandler(COMPLAINT_APIS.LODGED_OPEN_COMPLAINT, {
            ...
            <div className="max-w-6xl mx-auto px-4 py-6">
                ...
                <Tabs value={activeTab} className="w-full mt-5">
                    ...
                    <TabsContent value="step5">
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            <Button onClick={goBack} variant="primaryBorder">Back</Button>
                            <Button onClick={methods.handleSubmit(handleSubmitButton)}>Submit</Button>
                        </div>
                    </TabsContent>
                    <OtpPromptModal
                        open={showOtpModal}
                        ...
                        onConfirm={handleLodgeComplaint}
                    />
                </Tabs>
            </div>
        </FormProvider>
...
export default LodgeOpenComplaintMainForm;
