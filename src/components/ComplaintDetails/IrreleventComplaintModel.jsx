...
import { irrelevantDisposeSchema } from "@/schema/complaintSchema";
...
const IrreleventComplaintModel = ({ isOpen, complaint, onClose, onSubmit }) => {
  ...
  const {
    ...
    resolver: yupResolver(irrelevantDisposeSchema),
    defaultValues: {
      remarks: "",
      attachments: [],
    },
  ...
  const submitForm = async (data) => {
    ...
    formData.append("status", COMPLAINT_STATUS.FILE_NOT_RELAVENT);
    ...
    toast.success(response?.message || "Complaint marked as irrelevant");
    dispatch(fetchDashboardData(true))
    onSubmit?.();
  };

  return (
    <ModalWrapper isOpen={isOpen} onClose={handleClose} title="Dispose / Irrelevant Complaint">
...
export default IrreleventComplaintModel;
