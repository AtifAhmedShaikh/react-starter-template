...
import AdminRow from "./TableRow";
import { parsePageSafely } from "@/utils/pagination";

export default function AdminsTable({ apiFn, queryName = "admins", extraParams = {}, children }) {
    const [searchParams] = useSearchParams();
    const keyword = searchParams.get("keyword") || "";
    const page = parsePageSafely(searchParams.get("page"));
    console.log("Getting the page number", page);
    const { complaints = [], meta, loading, error } = useAdminsData({ queryName, apiFn, keyword, page, extraParams });

    return (
        <div className="space-y-4">
            <div className="flex justify-between m-1">
                <GlobalSearchInput placeholder="Search by name, cnic, designation" />
            </div>
            {
                children
            }
            <div className="rounded-md border md:!max-w-[calc(100vw-22rem)] max-w-[calc(100vw-3rem)] mx-auto ">
                <Table className={`overflow-hidden w-full  [&_th]:border-white [&_th]:!border-2 [&_th]:!p-2 [&_td]:!p-2 [&_td]:border-gray-100 [&_td]:!border-2`}>
                    ...
                    <TableBody>
                        ...
                        {!loading && !error && complaints.map((c, index) => {
                            const serialNumber = (meta?.limit || 10) * ((meta?.page || 1) - 1) + index + 1;
                            return <AdminRow key={c.id} serialNumber={serialNumber} user={c} searchKeyword={keyword} />;
                        })}
                    </TableBody>
                </Table>
            </div>
